# Question 3 — Exécuter un pas de calcul
# Lit les symboles sous chaque tête, cherche la transition, applique-la
...
def lire_symbole(config, numero_ruban):
    ...
    if position < 0 or position >= len(ruban):
        return "_"
...
def deplacer_tete(config, numero_ruban, mouvement):
    ...
    # Extension du ruban si la tête dépasse les limites
...
def un_pas_de_calcul(mt, config):
    # Lit les symboles, cherche la transition, retourne None si aucune
    symboles_lus = tuple(lire_symbole(config, i) for i in range(mt.nb_rubans))
...
if __name__ == "__main__":
    ...
    print("Avant :", config.etat, config.rubans)
    print("Après :", nouvelle.etat, nouvelle.rubans)
